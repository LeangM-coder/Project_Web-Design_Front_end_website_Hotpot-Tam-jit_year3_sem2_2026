...
function loginUser(telephone, password) {
  ...
  if (!savedUser) {
    showMessage("No account found. Please signup first.", "error");
    return;
  }

  if (telephone === savedUser.telephone && password === savedUser.password) {
    localStorage.setItem("isLoggedIn", "true");
    localStorage.setItem("profileName", savedUser.name);
    localStorage.setItem("profileTelephone", savedUser.telephone);
